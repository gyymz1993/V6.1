package com.lsjr.zizisteward.ymz.custom.dash;
...
public class CouponViewHelper {
    ...
    private boolean isShapeBg = false;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShapeBg) {
            RectF oval=new RectF(0,viewWidth,0,viewHeight);
            canvas.drawArc(oval, 10, 10, false, semicirclePaint);//小弧形
    ...
    public void setShapeBg(boolean shapeBg) {
        if (this.isShapeBg != shapeBg) {
            this.isShapeBg = shapeBg;
            calculate();
            view.invalidate();
        }
    }
