...
public class RoundedRectProgressBar extends View {
	...
	public RoundedRectProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		/* 获取自定义参数的颜色值 */
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.RoundedRectProgressBar, defStyle,
				0);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRoundRect(new RectF(0, 0, this.getMeasuredWidth(), this.getMeasuredHeight()), radius, radius,
				mPaint);
		// 进度条
		mPaint.setColor(barColor);
		mPaint.setStyle(Paint.Style.FILL);
		canvas.drawRoundRect(new RectF(0, 0, this.getMeasuredWidth() * progress / 100f, this.getMeasuredHeight()),
